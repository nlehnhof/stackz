      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              // Navigate to EditableRoomView
              await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => EditableRoomView(
                    room: room,
                    highlightItemName: highlightItemName,
                    scale: scale,
                    onShelfSelected: (shelf) {
                      debugPrint("Shelf selected: ${shelf.name}");
                    },
                    onAddLevel: (shelf) {
                      debugPrint("Add level to: ${shelf.name}");
                      shelf.levels.add(
                        Levels(id: shelf.levels.length.toString(), index: shelf.levels.length + 1, items: []),
                      );
                    },
                    onAddShelf: (shelf) {
                      debugPrint("Shelf added: ${shelf.name}");
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    for (int levelIndex = 0;
                        levelIndex < shelf.levels.length;
                        levelIndex++)
                      Positioned(
